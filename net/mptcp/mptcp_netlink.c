...
mptcp_nl_hash_find(struct net *net, u32 token)
{
	u32			hash = token % MPTCP_HASH_SIZE;
	...
	hlist_nulls_for_each_entry_rcu(meta_tp, node, &tk_hashtable[hash],
		...
		if (token == meta_tp->mptcp_loc_token &&
		    ...
		    refcount_inc_not_zero(&meta_sk->sk_refcnt))
			break;
		meta_sk = NULL;
	}
	rcu_read_unlock_bh();
...
mptcp_nl_put_subsk(struct sk_buff *msg, struct sock *sk)
{
	...
	if (unlikely(sk->sk_err) && meta_sk->sk_state == TCP_ESTABLISHED &&
	    nla_put_u8(msg, MPTCP_ATTR_ERROR, sk->sk_err))
		goto nla_put_failure;
...
{
	if (mptcp_nl_event_filter & MPTCPF_EVENT_CREATED)
		return;
...
mptcp_nl_pm_fully_established(struct sock *meta_sk)
{
	...
	if (mptcp_nl_event_filter & MPTCPF_EVENT_ESTABLISHED)
		return;
...
{
	if (mptcp_nl_event_filter & MPTCPF_EVENT_CLOSED)
		return;
...
static void
mptcp_nl_pm_new_subflow(struct sock *sk)
{
	struct sock *meta_sk = mptcp_meta_sk(sk);

	if (mptcp_nl_event_filter & MPTCPF_EVENT_SUB_CREATED)
		return;

	mptcp_nl_mcast(tcp_sk(meta_sk)->mpcb, sk, MPTCP_EVENT_SUB_CREATED);
}
...
mptcp_nl_pm_established_subflow(struct sock *sk)
{
	...
	if (mptcp_nl_event_filter & MPTCPF_EVENT_SUB_ESTABLISHED)
		return;
...
mptcp_nl_pm_delete_subflow(struct sock *sk)
{
	...
	if (mptcp_nl_event_filter & MPTCPF_EVENT_SUB_CLOSED)
		return;
		      ...
		      sa_family_t family, __be16 port, u8 id)
{
	...
	if (mptcp_nl_event_filter & MPTCPF_EVENT_ANNOUNCED)
		return;
...
mptcp_nl_pm_rem_raddr(struct mptcp_cb *mpcb, u8 id)
{
	...
	if (mptcp_nl_event_filter & MPTCPF_EVENT_REMOVED)
		return;
...
mptcp_nl_pm_prio_changed(struct sock *sk, int low_prio)
{
	...
	if (mptcp_nl_event_filter & MPTCPF_EVENT_SUB_PRIORITY)
		return;
...
static void
mptcp_nl_pm_subflow_error(struct sock *meta_sk, struct sock *sk)
{
	if (mptcp_nl_event_filter & MPTCPF_EVENT_SUB_ERROR)
		return;

	mptcp_nl_mcast(tcp_sk(meta_sk)->mpcb, sk, MPTCP_EVENT_SUB_ERROR);
}
...
mptcp_nl_genl_announce(struct sk_buff *skb, struct genl_info *info)
{
	...
	meta_sk = mptcp_nl_hash_find(genl_info_net(info), token);
	if (!meta_sk)
		return -EINVAL;
...
mptcp_nl_subsk_lookup(struct mptcp_cb *mpcb, struct nlattr **attrs)
{
	...
	case AF_INET: {
		...
		mptcp_for_each_sk_safe(mpcb, subsk, tmpsk) {
...
mptcp_nl_genl_destroy(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (subsk) {
		...
		if (tcp_need_reset(subsk->sk_state))
			tcp_send_active_reset(subsk, GFP_ATOMIC);
...
static int
mptcp_nl_genl_timeout(struct sk_buff *skb, struct genl_info *info)
{
	struct sock	*meta_sk, *subsk;
	struct mptcp_cb	*mpcb;
	int		ret = 0;
	...
	if (!info->attrs[MPTCP_ATTR_TIMEOUT])
		return -EINVAL;
	timeout = nla_get_u32(info->attrs[MPTCP_ATTR_TIMEOUT]);
	...
	subsk = mptcp_nl_subsk_lookup(mpcb, info->attrs);
	if (subsk)
		inet_csk(subsk)->icsk_user_timeout = msecs_to_jiffies(timeout);
	else
		ret = -EINVAL;
...
static int
mptcp_nl_genl_reset(struct sk_buff *skb, struct genl_info *info)
{
	u32		token;
	struct sock	*meta_sk;
	...
	mptcp_send_active_reset(meta_sk, GFP_KERNEL);
...
static int
mptcp_nl_dump_subflows(struct sk_buff *msg, struct mptcp_cb *mpcb, int *next_sf,
		       struct netlink_callback *cb)
{
	...
	mptcp_for_each_sk(mpcb, sk) {
		...
		hdr = genlmsg_put(msg, NETLINK_CB(cb->skb).portid,
				  ...
				  NLM_F_MULTI, MPTCP_EVENT_SUB_CREATED);
		...
		ret = nla_put_u32(msg, MPTCP_ATTR_TOKEN, mpcb->mptcp_loc_token);
		if (ret)
			goto nla_put_failure;

		ret = mptcp_nl_put_subsk(msg, sk);
		if (ret)
			goto nla_put_failure;
...
static int
mptcp_nl_genl_dump_sessions(struct sk_buff *msg, struct netlink_callback *cb)
{
	void *hdr;
	...
	/* Retrieve entries from the netlink_callback argument.
	 * * cb->args[0]: The main loop iterator.
	 * * cb->args[1]: The next connection that needs to be dumped.
	 * * cb->args[2]: 1 if we are dumping the current connection subflows.
	 * * cb->args[3]: The next subflow that needs to be dumped.
	 *
	 * They are all equal to 0 the first time the function is called for a
	 * new request.
	 */
	int	i			= cb->args[0];
	int	next_conn		= cb->args[1];
	int	dumping_subflows	= cb->args[2];
	int	next_sf			= cb->args[3];

	for (; i < MPTCP_HASH_SIZE; i++) {
		struct hlist_nulls_node *node;
		...
		hlist_nulls_for_each_entry_rcu(meta_tp, node,
			...
			if (!mptcp(meta_tp) || !mpcb || !mpcb->master_sk)
				continue;

			/* Send subflows of previous mptcp session */
			if (n == next_conn - 1 && dumping_subflows) {
				ret = mptcp_nl_dump_subflows(msg, mpcb, &next_sf,
							     cb);
				if (ret == 0)
					dumping_subflows = 0;
				else
			...
			hdr = genlmsg_put(msg, NETLINK_CB(cb->skb).portid,
					  cb->nlh->nlmsg_seq, &mptcp_genl_family,
					  NLM_F_MULTI, MPTCP_EVENT_CREATED);
			...
			ret = nla_put_u32(msg, MPTCP_ATTR_TOKEN,
					  mpcb->mptcp_loc_token);
			if (ret)
				goto nla_put_failure;

			ret = mptcp_nl_put_subsk(msg, mpcb->master_sk);
			if (ret)
				goto nla_put_failure;
...
static struct genl_ops mptcp_genl_ops[] = {
	...
	{
		.cmd	= MPTCP_CMD_SUB_TIMEOUT,
		.doit	= mptcp_nl_genl_timeout,
		.policy = mptcp_nl_genl_policy,
		.flags	= GENL_ADMIN_PERM,
	},
	{
		.cmd	= MPTCP_CMD_DUMP,
		.dumpit = mptcp_nl_genl_dump_sessions,
		.policy = mptcp_nl_genl_policy,
		.flags	= GENL_ADMIN_PERM,
	},
	{
...
static struct mptcp_pm_ops mptcp_nl_pm_ops = {
	...
	.new_subflow		= mptcp_nl_pm_new_subflow,
	...
	.subflow_error		= mptcp_nl_pm_subflow_error,
	.name			= "netlink",
	.owner			= THIS_MODULE,
};
